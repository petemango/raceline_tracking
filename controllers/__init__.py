...
def controller(
    ...
    current_index = get_closest_index(
        car_position, path, last_index=racetrack.last_idx, search_window=100
    )
    racetrack.last_idx = current_index

    cross_track_error, heading_error = get_track_errors(
        state, path, known_index=current_index, tangents=tangents
    )
    ...
    normalized_cte = max(
        0.0,
        (abs(cross_track_error) - cte_safe_threshold)
        / (cte_danger_threshold - cte_safe_threshold),
    )
    normalized_he = max(
        0.0,
        (abs(heading_error) - he_safe_threshold)
        / (he_danger_threshold - he_safe_threshold),
    )
    ...
    if safety_penalty > 0:
        ...
        governed_velocity = velocity_limit - safety_penalty * (
            velocity_limit - safe_recovery_velocity
        )
    ...
    if segment_distance > 1e-6:
        interpolation_factor = np.clip(
            np.dot(car_position - point_1, segment_vector / segment_distance)
            / segment_distance,
            0.0,
            1.0,
        )
    ...
    interpolated_curvature = (1.0 - interpolation_factor) * get_signed_curvature(
        index_1
    ) + interpolation_factor * get_signed_curvature(index_2)
    ...
    normalized_lateral_error = (
        cross_track_error / reference_width
    ) + 1.5 * heading_error
    derivative_lateral_error = (
        normalized_lateral_error - racetrack.previous_lateral_error
    ) / CONTROL_TIMESTEP
    ...
    feedback_steering = (base_kp * gain_scale * normalized_lateral_error) + (
        base_kd * gain_scale * derivative_lateral_error
    )
    ...
    desired_steering = float(
        np.clip(desired_steering, -max_steering_angle, max_steering_angle)
    )
    desired_velocity = float(
        np.clip(desired_velocity, float(parameters[2]), float(parameters[5]))
    )
