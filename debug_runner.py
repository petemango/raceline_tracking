...
if __name__ == "__main__":
    ...
    ims_hist = run_simulation(
        "racetracks/IMS.csv", "racetracks/IMS_raceline.csv", max_steps=100
    )
    ...
    for i in range(0, min(30, len(ims_hist)), 2):
        h = ims_hist[i]
        print(
            f"T={h['time']:.1f}s | V={h['vel']:.2f} | Steer={h['steer']:.3f} | CTE={h['cte']:.3f} | HE={h['he']:.3f}"
        )

    print("\n[Montreal Analysis]")
    montreal_hist = run_simulation(
        "racetracks/Montreal.csv", "racetracks/Montreal_raceline.csv", max_steps=3000
    )
    ...
    for t in range(0, 300, 20):
        ...
        if step_idx < len(montreal_hist):
            ...
            print(
                f"T={h['time']:.1f}s | V={h['vel']:.2f} | Steer={h['steer']:.3f} | CTE={h['cte']:.3f} | Idx={h['idx']}"
            )
    ...
    if max_cte_idx != -1:
        print(
            f"Max CTE found at Step {max_cte_idx} (T={montreal_hist[max_cte_idx]['time']:.1f}s)"
        )
    ...
    # Monza
    print("\n[Monza Analysis]")
    monza_hist = run_simulation(
        "racetracks/Monza.csv", "racetracks/Monza_raceline.csv", max_steps=3000
    )
    ...
    if max_cte_idx != -1:
        print(
            f"Max CTE found at Step {max_cte_idx} (T={monza_hist[max_cte_idx]['time']:.1f}s)"
        )
        start_window = max(0, max_cte_idx - 5)
