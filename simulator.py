...
class Simulator:
    def __init__(self, rt: RaceTrack):
        ...
        self.track_limit_violations = (
            0  # Total violations (deprecated if using current_lap_violations)
        )
    ...
    def run(self):
        try:
            ...
            self.axis.text(
                ...
                ("Final lap time: " if self.lap_finished else "Lap time: ")
                + f"{self.lap_time_elapsed:.2f}",
                horizontalalignment="right",
                verticalalignment="top",
                fontsize=8,
                color="Red",
            ...
            self.axis.text(
                ...
                "Track violations: " + str(self.current_lap_violations),
                horizontalalignment="right",
                verticalalignment="top",
                fontsize=8,
                color="Red",
    ...
    def update_status(self):
        ...
        if (
            hasattr(self.rt, "raceline")
            and hasattr(self.rt, "last_idx")
            and self.rt.last_idx is not None
        ):
            ...
            if (
                not hasattr(self, "initial_raceline_idx")
                or self.initial_raceline_idx is None
            ):
            ...
            if self.lap_started and not self.lap_finished:
                ...
                if (
                    self.max_progress_idx > 0.9 * n_points
                    and progress_idx < 0.1 * n_points
                ):
                    ...
                    self.lap_number += 1
                    print(
                        f"Lap {self.lap_number} finished in {self.lap_time_elapsed:.2f}s with {self.current_lap_violations} violations."
                    )
            ...
            progress = np.linalg.norm(
                self.car.state[0:2] - self.rt.centerline[0, 0:2], 2
            )
